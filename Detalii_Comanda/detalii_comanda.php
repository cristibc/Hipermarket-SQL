...
$statement = $pdo->prepare('SELECT * FROM detalii_comanda ORDER BY id_comanda ASC');
$statement->execute();
...
<?php foreach ($entry as $i => $entry): ?>
    <tr>
      ...
      <td><?php echo $entry['cod_produs'] ?></td>
      <td><?php echo $entry['cantitate'] ?></td>
      <td><?php echo $entry['pret'] ?></td>
      <td>
      <a href="editare.php?id_comanda=<?php echo $entry['id_comanda']?>&cod_produs=<?php echo $entry['cod_produs']?>" type="button" class="btn btn-sm btn-outline-secondary">Editare</a>
      <form style="display:inline-block" method="post" action="sterge.php">
        ...
        <input type="hidden" name="cod_produs" value="<?php echo $entry['cod_produs'] ?>">
