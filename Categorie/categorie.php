...
// if ($cautare) { 
//   $statement = $pdo->prepare('SELECT * FROM categorie WHERE id_categorie LIKE :id_categorie ORDER BY id_categorie ASC');
//   $statement->bindValue(':id_categorie', "%$cautare%");
// } else {
//   $statement = $pdo->prepare('SELECT * FROM categorie ORDER BY id_categorie ASC');
// }
...
<html lang="en">
  ...
  <div class="style">
    ...
    <p>
        <a href="adaugare.php" class="btn btn-success">Adaugă Categorie</a>
    </p>
...
<?php foreach ($entry as $i => $entry): ?>
    <tr>
      <th><?php echo $entry['id_categorie'] ?></th>
      <td><?php echo $entry['nume'] ?></td>
      <td><?php echo $entry['descriere'] ?></td>
      <td>
      <a href="editare.php?id_categorie=<?php echo $entry['id_categorie'] ?>" type="button" class="btn btn-sm btn-outline-secondary">Editare</a>
      <form style="display:inline-block" method="post" action="sterge.php">
        <input type="hidden" name="id_categorie" value="<?php echo $entry['id_categorie'] ?>">
