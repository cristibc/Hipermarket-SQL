...
// if ($cautare) { 
//   $afisare = $pdo->prepare('SELECT * FROM client WHERE nume LIKE :nume ORDER BY id_client ASC');
//   $afisare->bindValue(':nume', "%$cautare%");
// } else {
//   $afisare = $pdo->prepare('SELECT * FROM client ORDER BY id_client ASC');
// }
...
<html lang="en">
  ...
  <div class="style">
    ...
    <p>
        <a href="adaugare.php" class="btn btn-success">Adaugă Client</a>
    </p>
...
<?php foreach ($entry as $i => $entry): ?>
    <tr>
      <th><?php echo $entry['id_client'] ?></th>
      <td><?php echo $entry['nume'] ?></td>
      <td><?php echo $entry['prenume'] ?></td>
      <td><?php echo $entry['nr_telefon'] ?></td>
      <td><?php echo $entry['adresa'] ?></td>
      <td><?php echo $entry['oras'] ?></td>
      <td><?php echo $entry['email'] ?></td>
      <td><?php echo $entry['sex'] ?></td>
      <td>
      <a href="editare.php?id_client=<?php echo $entry['id_client'] ?>" type="button" class="btn btn-sm btn-outline-secondary">Editare</a>
      <form style="display:inline-block" method="post" action="sterge.php">
        <input type="hidden" name="id_client" value="<?php echo $entry['id_client'] ?>">
